...
cat<-as.factor(rep(c("Fill","Empty"),each=2,times=25))
base<-as.factor(rep(c("Wide Base","Wide Top"),each=20,len=100))
plot_dd<-data.frame(nums,cat,base)

ggplot(plot_dd,aes(cat,nums,group=base))+geom_bar(stat = "identity",position = "dodge")
```
